...
export class InvitesComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    // })
    $(function() {
      var a = document.getElementById('datetimes') as HTMLFormElement;
      a.daterangepicker({
        timePicker: true,
  ...
  inviteVisitor() {
    ...
    if ($('#Fullname').val() === '' || $('#Fullname').val() === null) {
      ...
      $('.Fullname').html('Enter Full Name');
    } else {
      $("#Fullname").removeClass('error');
      $('.Fullname').html('');
    }
    if ($("#inviteEmail").val() !== '') {
      if (!emailPattern.test($("#inviteEmail").val().toString())) {
        err++;
        $("#inviteEmail").addClass('error');
        $('.inviteEmail').html('Enter Email');
      } else {
    ...
    if ($("#inviteContact").val() !== '') {
      if (!mobilePattern.test($("#inviteContact").val().toString())) {
        err++;
        $("#inviteContact").addClass('error');
        $('.inviteContact').html('Enter Correct Mobile number');
      } else {
        $("#inviteContact").removeClass('error');
        $('.inviteContact').html('');
      }
    } else {
      err++;
    ...
    if ($('#invitePurpose').val() === '' || $('#invitePurpose').val() === null) {
      err++;
      $("#invitePurpose").addClass('error');
      $('.invitePurpose').html('Enter Purpose');
    } else {
      $("#invitePurpose").removeClass('error');
      $('.invitePurpose').html('');
    }
    if ($('#inviteHost').val() === '' || $('#inviteHost').val() === null) {
      err++;
      $("#inviteHost").addClass('error');
      $('.inviteHost').html('Select Host');
    } else {
      $("#inviteHost").removeClass('error');
      $('.inviteHost').html('');
    }
    if ($('#inviteArrivalDate').val() === '' || $('#inviteArrivalDate').val() === null) {
      err++;
      $("#inviteArrivalDate").addClass('error');
      $('.inviteArrivalDate').html('Select Arrival Date');
    } else {
      $("#inviteArrivalDate").removeClass('error');
      $('.inviteArrivalDate').html('');
    }
    ...
    if (err == 0) {
      ...
      var obj = {
        ...
        "SendEmailInvite": this.emailPreview == true ? "True":"False",
        "Hosts":
          [
            {
              "Name": "Sanjay",
  ...
  getUsers() {
    const _self = this;
    $.ajax({
      url: "https://apis.justlogindevelopment.xyz/visitors/v2/details?VisitorName=ramo&VisitorEmail=patil",
      type: 'GET',
      headers: {
        "Content-Type": "application/json",
        "Authorization": "Bearer " + this.ApiService.Token
  ...
  getVisitor(data) {
    ...
    const _self = this;
    $.ajax({
      url: "https://apis.justlogindevelopment.xyz/visitors/v2/details/" + id,
      type: 'GET',
      headers: {
        "Content-Type": "application/json",
        "Authorization": "Bearer " + this.ApiService.Token
  ...
  invitedVisitors() {
    ...
    var abc = $.ajax({
      url: "https://apis.justlogindevelopment.xyz/visitors/v2/invitation",
      type: 'GET',
      headers: {
        "Authorization": "Bearer " + this.ApiService.Token
      },
  ...
  getHost(){
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        $('#inviteHost').empty();
        for(var i =0;i<res.length;i++){
          $('#inviteHost').append('<option value="'+res[i]['email']+'">'+res[i]['fullName']+'</option>')
        }
        
      }, error(err) {
        _self.toaster.error('Sorry there was a problem while loading Host. Please try again.');
        console.log('/visitors/v2/details/---' + err);
      }
    });
  }
  getLocation(){
    let _self = this;
    $.ajax({
      url: 'https://apis.justlogindevelopment.xyz/visitors/v2/setup',
      type: 'GET',
      headers: {
        "Content-Type": "application/json",
        "Authorization": "Bearer " + this.ApiService.Token
      ...
      success: function (res) {
        var obj = res;
        $('#inviteLocation').empty();
        for(var i =0;i<res['setups'].length;i++){
          if(res['setups'][i]['location'] && res['setups'][i]['location']['name'])
          $('#inviteLocation').append('<option value="'+res['setups'][i]['location']['name']+'">'+res['setups'][i]['location']['name']+'</option>')
        }
  ...
  dashboardChecked(isChecked,id){
    var temp_checkedIds=[];
    if(isChecked){
      this.checkedIds.push(id);
    }else{
      var arr = this.checkedIds.filter(function(item) {
         if(item !== id){
          temp_checkedIds.push(item);
         }
      })
      this.checkedIds = temp_checkedIds;
    }
  }
  deleteInvite(){
    var _self = this;
    if(this.checkedIds.length > 0){
      $.ajax({
        url: 'https://apis.justlogindevelopment.xyz/visitors/v2/entry/DeleteInvitations',
        type: 'POST',
        ...
        dataType: "json",
        success: function (res) {
          _self.toaster.success('Deleted successfully.');
          console.log(res);
        },
        error(err) {
          console.log(err);
          _self.toaster.error('Sorry there was a problem while deleting. Please try again.');
        }
      });
    }else{
      _self.toaster.error('Please select records to delete.');
    }
  }
  checkIn(){
    var _self = this;
    if(this.checkedIds.length > 0){
      $.ajax({
        url: 'https://apis.justlogindevelopment.xyz/visitors/v2/entry/Invitations',
        type: 'POST',
        data: JSON.stringify(this.checkedIds),
        headers: {
          "Content-Type": "application/json",
      ...
      _self.toaster.error('Please select records to invite.');
    }
  }
}
